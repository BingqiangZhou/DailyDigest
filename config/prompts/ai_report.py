...
Step 1: 浏览所有文章，识别3-5个最重要的AI发展动态
Step 2: 将相关文章归入同一主题（同一事件的多篇报道应合并分析，而非逐条列出）
Step 3: 识别跨文章的趋势模式（多家公司发布同类产品？某项技术从研究走向应用？行业格局变化？）
Step 4: 按以下结构输出Markdown格式报告
Step 5: 自检——确认所有引用的文章都存在于原文列表中，确认没有编造统计数字或引用
...
Step 1: Scan all articles and identify the 3-5 most significant AI developments
Step 2: Group related articles under the same theme (multiple reports about the same event should be merged, not listed separately)
Step 3: Identify cross-article trend patterns (multiple companies pursuing the same approach? A technology moving from research to production? Industry landscape shifts?)
Step 4: Output the Markdown report following the structure below
Step 5: Self-check — verify all cited articles exist in the source list, confirm no fabricated statistics or quotes
